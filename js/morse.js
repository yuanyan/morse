...
function tieSpeeds() {
	var wpm = parseInt(document.mform.wpm.value);
	var fwpm = parseInt(document.mform.fwpm.value);
	document.mform.fwpm.value = fwpm > wpm ? wpm : fwpm;
	return true;
}
function translateM() {
	var input = document.mform.input.value;
	var transError = document.getElementById("transError");
	if (len(input) == 0) {
		document.mform.output.value = '';
		morse = '';
		message = '';
		transError.innerHTML = '';
		return;
	}
	if (document.mform.useprosigns.checked) {
		var output = String(convertPro(input));  //TODO why String?
	} else {
		var output = String(convert(input));
	}
	document.mform.output.value = output;
...
function playM() {
	...
	var wpm = parseInt(document.mform.wpm.value);
	var fwpm = parseInt(document.mform.fwpm.value);
	var pitch = parseInt(document.mform.pitch.value);
	var volume = parseFloat(document.mform.volume.value);
